...
import 'package:card_game/features/online/room/models/seat_player.dart';
import 'package:card_game/features/online/room/repositories/online_game_services.dart';
...
class OnlineGameController extends BaseController {
  OnlineGameController(this.roomId, this.myPlayerId, this.snapshotString) {
    snapshot = RoomLobbySnapshot.fromRawJson(snapshotString).obs;
  }
  final String roomId;
  ...
  late final Rx<RoomLobbySnapshot> snapshot;
  RxMap roomState = {}.obs;
  RoomLobbySnapshot? get room => snapshot.value;
  bool get gameEnded => roomState['game_status'] == 'finished';
  ...
  final openCardKey = GlobalKey();
  final handKey = GlobalKey();
  final cardFlight = Rx<CardFlight?>(null);
  ...
  void sortCardsById() {
    // myHand.sort((a, b) => a.rank.compareTo(b.pilePosition ?? 0));
  ...
  List<SeatPlayer?> get seatPlayers {
    ...
    final myIndex = players.indexWhere(
      (e) => e.playerId == snapshot.value.localPlayerId,
    );
    ...
    // Rotate so I'm always first (bottom)
    final rotated = List<RoomLobbyPlayer>.generate(
      players.length,
      (i) => players[(myIndex + i) % players.length],
    );

    final seats = List<SeatPlayer?>.filled(4, null);

    switch (rotated.length) {
      case 2:
        seats[0] = SeatPlayer(seat: SeatPosition.bottom, player: rotated[0]);

        seats[2] = SeatPlayer(seat: SeatPosition.top, player: rotated[1]);
        break;

      case 3:
        seats[0] = SeatPlayer(seat: SeatPosition.bottom, player: rotated[0]);

        seats[1] = SeatPlayer(seat: SeatPosition.left, player: rotated[1]);

        seats[2] = SeatPlayer(seat: SeatPosition.top, player: rotated[2]);
        break;
      ...
      default:
        ...
        seats[3] = SeatPlayer(seat: SeatPosition.right, player: rotated[3]);
    }
  ...
  Future<void> pickOpenCard({required CardModel card}) async {
    ...
    await execute(() async {
      ...
      if (result['success'] == true) {
        ...
        cardFlight.value = CardFlight(
          type: CardAnimationType.drawFromOpen,
          card: card,
        );
      }
    });
  ...
  Future<void> sort() async {
    try {
      ...
      final List<String> mapListIds = [];

      for (CardModel card in myHand) {
        log('cards id ${card.id} ${card.pilePosition}');
        mapListIds.add(card.id);
      }
      await execute(
        () => _service.reorderHand(roomId: roomId, cardIds: mapListIds),
      );
    } catch (e) {
  ...
  Future<void> drawFromDeck() async {
    ...
    await execute(() async {
      ...
      if (result['success'] == true) {
        log('Successfully drew card: $result');

        cardFlight.value = CardFlight(
          type: CardAnimationType.drawFromDeck,
  ...
  void clearData() {
    ...
    rankings.clear();
    fourthCard.clear();
    joker = Rx<CardModel?>(null);
    selectedCard = Rx<CardModel?>(null);
    openCard = Rx<CardModel?>(null);
    discardPile.clear();
    snapshot;
    roomState.value = {};
    jokerUnlocked.value;
