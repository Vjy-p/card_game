...
class OnlineDeclareBottomsheet extends StatefulWidget {
  const OnlineDeclareBottomsheet({
    super.key,
    required this.cards,
    required this.fourthCard,
  });
  final List<CardModel> cards;
  final List<CardModel> fourthCard;

  @override
  State<OnlineDeclareBottomsheet> createState() =>
      _OnlineDeclareBottomsheetState();
}

class _OnlineDeclareBottomsheetState extends State<OnlineDeclareBottomsheet> {
  late List<CardModel> availableCards;
  // Updated to hold CardModel objects instead of ints
  List<List<CardModel>> cardSets = List.generate(4, (_) => []);
  ...
  void initState() {
    ...
    if (widget.fourthCard.length == 4) {
      for (CardModel card in widget.fourthCard) {
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ActionButton(
                        onPressed: () async {
                          final bool resp = await controller.endGame(
                            sets: cardSets,
                          );
                          if (resp) {
    ...
    return Material(
      ...
      child: Transform.rotate(
        ...
        child: Container(
          ...
          // Using your OnlineCardFace widget inside the container
          child: ClipRRect(
            borderRadius: BorderRadius.circular(8),
            child: OnlineCardFace(card: card),
          ),
