...
class OnlineFourthCardBottomsheet extends StatefulWidget {
  const OnlineFourthCardBottomsheet({super.key, required this.cards});
  final List<CardModel> cards;

  @override
  State<OnlineFourthCardBottomsheet> createState() =>
      _OnlineFourthCardBottomsheetState();
}

class _OnlineFourthCardBottomsheetState
    extends State<OnlineFourthCardBottomsheet> {
  late List<CardModel> availableCards;
  // Updated to hold CardModel objects instead of ints
  List<CardModel> cardSets = [];
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ActionButton(
                        onPressed: () async {
                          final bool resp = await controller.unlockJoker(
                            cardSets,
                          );
                          if (resp) {
                            log('4th card succesfull');
                            Get.back();
                          } else {
                            customToast(message: 'Wrong 4th cards');
                          }
                        },
                        label: 'Declare',
          ...
          child: Center(
            child: cardSets.isEmpty
                ...
                : Wrap(
                    ...
                    runSpacing: 4,
                    alignment: WrapAlignment.center,
                    children: cardSets.map((card) {
                      return GestureDetector(
                        onTap: () {
                          // Tap to return card to pile
                          setState(() {
                            cardSets.remove(card);
  ...
  Widget _buildDraggableCard(CardModel card, int index) {
  ...
  Widget _buildMiniCard(CardModel card) {
      ...
      builder: (context, double scale, child) {
        return Transform.scale(
          ...
          child: Container(
            ...
            child: AspectRatio(
              aspectRatio: 5 / 7,
              child: OnlineCardFace(card: card),
            ),
          ),
        );
      },
